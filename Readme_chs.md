...
# 简介
这是一个关于连接 Google 官方 API 的简单 MVP 演示项目，能从全球范围内使用，但依然遵守其政策。如果你想尝试用 Google Veo3 生成视频，这个项目应该是你入门实践的最佳选择。
...
## 第九步
杀掉“第五步”中运行的命令行进程。然后修改 `system service` 文件夹下的系统服务，需将其中的 `WorkingDirectory` 和 `ExecStart` 替换为你自己的配置。之后把这个文件放到 /etc/systemd/system/ 目录下（我的系统是 Debian，所以放在这里，如果你的系统不同，可能需要自己查一下对应的路径）。
